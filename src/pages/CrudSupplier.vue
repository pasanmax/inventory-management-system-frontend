	...
	<div class="grid">
		<div class="col-12">
			<div class="card">
				...
				<DataTable ref="dt" :value="supplier" v-model:selection="selectedSuppliers" dataKey="id" :paginator="true" :rows="10" :filters="filters"
							paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown" :rowsPerPageOptions="[5,10,25]"
							currentPageReportTemplate="Showing {first} to {last} of {totalRecords} Suppliers" responsiveLayout="scroll">
					<template #header>
				...
				<!--	<img :src="'images/product/' + product.image" :alt="product.image" v-if="product.image" width="150" class="mt-0 mx-auto mb-5 block shadow-2" />
                    ...
                    <div class="field">
						...
						<InputText id="name" v-model.trim="supplier.name" required="true" autofocus :class="{'p-invalid': submitted && !supplier.name}" />
						<small class="p-invalid" v-if="submitted && !supplier.name">Name is required.</small>
					</div>
					<div class="field">
						<label for="description">Description</label>
						<Textarea id="description" v-model="supplier.description" required="true" rows="3" cols="20" />
					</div>

					<div class="field">
						<label for="Status" class="mb-3">Status</label>
						<Dropdown id="Status" v-model="supplier.status" :options="statuses" optionLabel="label" placeholder="Select a Status">
					...
					<div class="field">
						...
						<div class="formgrid grid">
							<div class="field-radiobutton col-6">
								<RadioButton id="category1" name="category" value="Accessories" v-model="supplier.category" />
...
export default {
	...
	mounted() {
		this.supplierService.getSupplier().then(data => this.suppliers = data);
	},
	methods: {
		...
		saveSuppliert() {
			...
			if (this.supplier.id) {
				this.supplier.status = this.supplier.status.value ? this.supplier.status.value: this.supplier.status;
				this.supplier[this.findIndexById(this.supplier.id)] = this.supplier;
				this.$toast.add({severity:'success', summary: 'Successful', detail: 'Product Updated', life: 3000});
				}
				else {
					this.supplier.id = this.createId();
					this.supplier.code = this.createId();
					//this.product.image = 'product-placeholder.svg';
					this.supplier.status = this.supplier.status ? this.supplier.status.value : 'Active';
					this.supplier.push(this.supplier);
		...
		deleteSupplier() {
			this.supplier = this.supplier.filter(val => val.id !== this.supplier.id);
		...
		deleteSelectedSuppliers() {
			this.supplier = this.supplier.filter(val => !this.selectedSuppliers.includes(val));
